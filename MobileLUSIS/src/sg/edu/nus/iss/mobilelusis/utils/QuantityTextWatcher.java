  package sg.edu.nus.iss.mobilelusis.utils;

import sg.edu.nus.iss.mobilelusis.model.StationeryItem;
import android.text.Editable;
import android.text.TextWatcher;

public class QuantityTextWatcher implements TextWatcher {

	private StationeryItem stationeryItem = null;
	public QuantityTextWatcher(StationeryItem stationeryItem) {
		super();
		this.stationeryItem = stationeryItem;
	}

	@Override
	public void afterTextChanged(Editable s) {
	}

	@Override
	public void beforeTextChanged(CharSequence s, int start, int count, int after) {
	}

	@Override
	public void onTextChanged(CharSequence s, int start, int before, int count) {
		String newQty = s.toString();
		try {
			Integer newQuantity = Integer.valueOf(newQty);
			stationeryItem.setQuantity(newQuantity.intValue());
			
		} catch (NumberFormatException nfe) {
			nfe.printStackTrace();
		}
		
	}
	
	
}
